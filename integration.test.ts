...
async function runIntegrationTest(testCase: TestCase): Promise<void> {
  ...
  try {
    ...
    if (result.success === true) {
      console.log(
        `   ✅ Success: RTS returned ${
          result.qualifiedSegments?.length || 0
        } segments`,
      );
      if (result.metadata?.flagResult) {
        console.log(
          `   🚩 Flag result: ${result.metadata.flagResult.variationKey} (enabled: ${result.metadata.flagResult.enabled})`,
        );
      }
...
async function main() {
  ...
  if (!serverRunning) {
    console.log(
      "❌ Server is not running. Please start it first with: deno task dev",
    );
    console.log(
      "   Then run this integration test with: deno run --allow-net integration.test.ts",
    );
    Deno.exit(1);
  }
  ...
  console.log(
    '   - Replace "YOUR_REAL_SDK_KEY_HERE" with your actual Optimizely SDK key',
  );
  console.log(
    '   - Replace "your-flag-key" with an actual flag key for flag evaluation tests',
  );
  console.log(
    "   - Set OPTIMIZELY_SDK_KEY environment variable to test without passing keys in requests",
  );
  console.log("\n🎯 Expected behavior with real SDK key:");
  console.log(
    "   - The function should successfully import the v6.0.0 universal bundle",
  );
  console.log(
    "   - fetchQualifiedSegments should return an array of segment names",
  );
