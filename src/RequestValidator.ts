...
export class RequestValidator {
  static async parseAndValidate(request: Request): Promise<RTSTestRequest> {
    ...
    try {
      ...
      throw new Error(`Failed to parse request body: ${parseError instanceof Error ? parseError.message : 'Invalid JSON'}`);
    }
  ...
  static validateSdkKey(): string {
    ...
    if (!sdkKey) {
      ...
      throw new Error('SDK key is required (set OPTIMIZELY_SDK_KEY env var or include in request body)');
    }
